...
在前三章中我们的案例大量使用到了`Thread`这个类，通过其原始API，对其进行创建、启动、中断、终止、取消以及异常处理，这样的写法不仅不够优雅（对接下来这篇，我称其为`.NET现代化并行编程体系`中出现的API而言），并且大部分的接口都是极度复杂和危险的。很幸运，如今.NET已经提供，并且普及了一系列多线程API来帮助我们，优雅且安全的达到相同的目的。
...
使用`PFX`前需要检查是否真的有必要并行（串行是最快的），经过对阿姆达尔定律的了解，我们可以看出，并非使用更多的处理器，性能就能随之水涨船高。如果顺序执行的代码段占了这个工作的三分之二，即使用无数核心，也无法获得哪怕0.5倍的性能提升。
...
只需要在集合后面加个`AsParallel()`，就可以像平时写LNQ一样继续使用了，Framework会自动的进行工作分解，然后调用线程执行任务，最终将各个线程的结果整理并返回给你。
...
PLINQ 切分多个任务，将任务交由线程池线程处理，线程池的任务是支持取消令牌（安全取消协作模式），PLINQ 同理支持`CancellationToken`。我们使用之前使用的PLINQ 找素数案例，然后起一个任务，在2ms后取消。
...
- 如果输入序列可以通过索引访问（`Array`或`IList<T>`的实现），PLINQ 选用范围分区，范围分区会为每个工作线程平均的分配元素。如果序列中每个元素处理时间接近，那范围分区是效率最高的分区策略，因为他几乎没有额外的分区成本。
  ...
  ![image-20221217140708051](.\images\image-20221217140708051.png)
...
- 否则，PLINQ 选用块分区。块分区定期从序列中抓取小块（一个或两个元素），块大小随查询的进度逐渐变大。如果一个工作线程恰好拿到了一些相对容易的块，它最终会获取更多块，这个设计可以使核心负载均衡。但由于线程从序列中抓取块是需要同步的，因此会有一定的开销和竞争。
  ...
  ![image-20221217140555594](.\images\image-20221217140555594.png)
...
​	`Task` 家族均在命名空间`System.Threading.Tasks`下，`Task` 是Framework 4.0首次引入的**基于任务的异步模式 （TAP）**的核心成员。`Task` 将由线程池中的线程执行，避免了启动独立线程带来的开销。还可以使用`Status` `IsCanceled`、 `IsCompleted`和 `IsFaulted`属性来确定任务的状态。同时`Task`还提供了泛型版本，以供返回值。

​	`Task` 为多核而生，他能以一种极为简单高效的方式使用线程池，还提供了大量强大的API来辅助你完成工作，甚至提供了原生语法级别的async await来帮助你轻松构建一个异步任务。任意的并行述求都能通过`Task` 完成，因为它处于的层次更底层。事实上，`PLINQ` 和`Parallel` 的内部都是基于`Task`来构建的。

> 前面讲的`PLINQ` 和`Parallel` 都属于结构化并行，而`Task` 是非结构化的。即，使用`Task` 我们需要自己去拆解、执行和收集。

### 创建与启动

我们平时用的最多的是`Task.Run` 是从 .NET Framework 4.5 开始的，默认就启动任务，不需要手动调用Start()。

另一种是`Task.Factory.StartNew` 是 .NET Framework 4 中启动任务的最常见方法
...
输出：t4的以同步方式运行，它会在主线程上执行。 其余任务通常在一个或多个线程池线程上异步执行。

```shell
Task id=1, obj=beta, Thread id=7
t1 has been launched. (Main Thread id=17)
Task id=2, obj=alpha, Thread id=7
Task id=3, obj=delta, Thread id=8
Task id=4, obj=gamma, Thread id=17
```

### 等待

对单个`Task` 只需要调用`Wait` 方法，阻塞调用线程，直到任务完成。
...
默认情况下等待是无休止的，我们可以传递一个超时时间来自动解除调用线程的阻塞：

```c#
task.Wait(1000); // 超过1秒就不等了
```

那么于此对应，任务的状态也就不再是`RanToCompletion`了

```shell
task status: Running
```

`Wait` 还支持传递`CancellationToken`，一旦令牌被取消，就会抛出`OperationCanceledException` 异常
...
`Task` 还提供一些用于等待的方法（是不是想起了`WaitHandle`？）：

- `Task.WaitAny`：等待一组任务中的任意一个任务完成
- `Task.WaitAll`：等待一组任务全部完成

>要注意的是这些的任务必须先启动哦，否则会永远陷入阻塞。
>
>`Task` 还提供`WhenAny`、`WhenAll`，功能与上面的相同，不过返回的是一个`Task`
